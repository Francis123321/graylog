...
export default class SeriesConfiguration extends React.Component {
  ...
  render() {
    ...
    const validationHint = isValid ? null : <> <strong>名称必须是唯一的。</strong></>;
    ...
    return (
      <span>
        <FormGroup validationState={isValid ? null : 'error'}>
          ...
          <HelpBlock>出现在图表中的系列名称。{validationHint} </HelpBlock>
        </FormGroup>
        <div className="pull-right" style={{ marginBottom: '10px' }}>
          <Button bsStyle="success" disabled={!isValid} onClick={this._onSubmit}>完毕</Button>
        </div>
      </span>
    );
  }
